...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v) {
        ...
        //reset canvas
        window->DrawList->AddRectFilled(ImVec2(newRect.Min.x - 1.f, newRect.Min.y - 1.f), ImVec2(newRect.Max.x + .5f, newRect.Max.y + 1.f), check_col, 10);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x / 2, grab_bb.Min.y), grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
